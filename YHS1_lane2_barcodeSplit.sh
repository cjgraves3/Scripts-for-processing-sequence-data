...
#Example file name YHS2_lane1_read2_N7index$EXT
LIB='YHS1'
LANE='lane2'
EXT='.fastq.gz'
...
#When script was run line 35 had a typo and wrote data to an output file labeled with '_517' instead of '_N517'. I've subsequently corrected the script,
#but I only noticed far downstream and it would take weeks to repeat from this step. As a consequence, all reads with barcode N517 in lane 2 are absent from
#the dataset used in my analyses. Luckily, these samples are C1-C10 at timepoint 1 and therefore the reduced coverage probably has very little effect on 
#subsequent analyses to identify adaptive mutations